...
(
    select *
        , ROW_NUMBER() over(partition by s.player,s.team    order by s.activity_time  asc) as RowNum

    from  main.activities s
...
,sum(case when activity_type LIKE '%ADDED' then points_1 END) - COALESCE(sum(case when activity_type='DROPPED' then points_1  END ),0) as test
FROM segments
group by 1,2